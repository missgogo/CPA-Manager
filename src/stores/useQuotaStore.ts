...
import type { MonitoringAccountQuotaState } from '@/types';
...
interface QuotaStoreState {
  ...
  monitoringAccountQuota: Record<string, MonitoringAccountQuotaState>;
  setAntigravityQuota: (updater: QuotaUpdater<Record<string, AntigravityQuotaState>>) => void;
  setClaudeQuota: (updater: QuotaUpdater<Record<string, ClaudeQuotaState>>) => void;
  setCodexQuota: (updater: QuotaUpdater<Record<string, CodexQuotaState>>) => void;
  setGeminiCliQuota: (updater: QuotaUpdater<Record<string, GeminiCliQuotaState>>) => void;
  setKimiQuota: (updater: QuotaUpdater<Record<string, KimiQuotaState>>) => void;
  setMonitoringAccountQuota: (
    updater: QuotaUpdater<Record<string, MonitoringAccountQuotaState>>
  ) => void;
...
export const useQuotaStore = create<QuotaStoreState>()(
  persist(
    (set) => ({
      ...
      setMonitoringAccountQuota: (updater) =>
        set((state) => ({
          monitoringAccountQuota: resolveUpdater(updater, state.monitoringAccountQuota)
        })),
      clearQuotaCache: () =>
        set({
          antigravityQuota: {},
